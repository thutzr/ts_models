...
from .common import GBN, Entmax15, initialize_glu
...
class Danet(nn.Module):
    def __init__(self, input_size, num_layers, hidden_size, k=1, virtual_batch_size=10000, dropout=0.1):
        super(Danet, self).__init__()
        params = {'base_outdim': hidden_size, 'k': k, 'virtual_batch_size': virtual_batch_size,
                  'fix_input_dim': input_size, 'drop_rate': dropout}
        self.init_layer = BasicBlock(input_size, **params)
        self.lay_num = num_layers
        self.layer = nn.ModuleList()
        for i in range((num_layers // 2) - 1):
            self.layer.append(BasicBlock(hidden_size, **params))
