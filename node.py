...
from .common import sparsemax, sparsemoid, ModuleWithInit, check_numpy
from warnings import warn


class ODST(ModuleWithInit):
    def __init__(self, input_size, num_trees, depth=6, tree_dim=1, flatten_output=True,
        ...
        self.feature_selection_logits = nn.Parameter(
            torch.zeros([input_size, num_trees, depth]), requires_grad=True
        )
...
class Node(nn.Module):
    def __init__(self, input_size, num_trees, num_layers, depth=6, tree_dim=1, max_features=None,
                 dropout=0.0, flatten_output=True, Module=ODST, **kwargs):
        super().__init__()
        layers = []
        for i in range(num_layers):
            oddt = Module(input_size, num_trees, depth=depth, tree_dim=tree_dim, flatten_output=True, **kwargs)
            input_size = min(input_size + num_trees * tree_dim, max_features or float('inf'))
            layers.append(oddt)
        ...
        #     self.final_linear = nn.Linear(self.tree_dim, 1)
        # else:
        #     self.final_linear = nn.Linear(self.num_layers * self.layer_dim * self.tree_dim, 1)
    ...
    def forward(self, x):
        ...
        # return self.final_linear(outputs).squeeze(-1)
