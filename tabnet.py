...
from .common import Sparsemax, Entmax15, GBN
...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        for step in range(self.n_steps):
            ...
            # output
            self.group_attention_matrix = self.group_attention_matrix.to(M.device)
...
class TabNetNoEmbeddings(torch.nn.Module):
    def __init__(
        ...
        mask_type : str
            Either "sparsemax" or "entmax" : this is the masking function to use
        group_attention_matrix : torch matrix
            如果实现知道feature的分组，可以用来对feature分组
            Matrix of size (n_groups, input_dim), m_ij = importance within group i of feature j
        """
    ...
    def forward(self, x):
        ...
        return out, -self.lambda_sparse * M_loss
...
class Tabnet(torch.nn.Module):
    def __init__(
        ...
        if self.n_independent == 0 and self.n_shared == 0:
            raise ValueError("n_shared and n_independent can't be both zero.")
        # TODO: add group attention matrix
        self.group_matrix = create_group_matrix(group_attention_matrix, self.input_dim).to(device)
        
        self.virtual_batch_size = virtual_batch_size
        self.embedder = EmbeddingGenerator(input_size,
                                           cat_dims,
                                           cat_idxs,
                                           cat_emb_dim,
                                           self.group_matrix)
