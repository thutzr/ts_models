...
class Tabtransformer(nn.Module):
    def __init__(
        ...
        shared_embed_dim = 0 if not use_shared_categ_embed else int(embed_dim // shared_categ_dim_divisor)

        self.category_embed = nn.Embedding(total_tokens, embed_dim - shared_embed_dim)
        ...
        if self.num_continuous > 0:
            if exists(continuous_mean_std):
                assert continuous_mean_std.shape == (num_con, 2), f'continuous_mean_std must have a shape of ({num_con}, 2) where the last dimension contains the mean and variance respectively'
            self.register_buffer('continuous_mean_std', continuous_mean_std)

            self.norm = nn.LayerNorm(num_con)
        ...
        input_size = (embed_dim * self.num_categories) + num_con
